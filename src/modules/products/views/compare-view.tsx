...
export default function CompareView() {
  ...
  if (compare.items.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-8 bg-slate-50">
        <div className="w-24 h-24 bg-white shadow-xl shadow-slate-100 rounded-3xl flex items-center justify-center mb-6 relative">
          <div className="absolute inset-0 bg-indigo-500/5 rounded-3xl animate-ping opacity-50" />
          <Layers size={44} className="text-indigo-500" />
        </div>
        <h2 className="text-2xl font-black text-slate-900 mb-2 tracking-tight">Comparison list is empty</h2>
        <p className="text-slate-500 mb-8 max-w-sm text-center text-sm font-medium">
          Choose and add up to 4 products to compare their detailed specifications side by side.
        </p>
        <Link href="/products" className="inline-flex items-center gap-2 px-8 h-13 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all hover:-translate-y-0.5 active:translate-y-0 shadow-lg shadow-indigo-600/15">
          Browse Products
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50/50 py-6 sm:py-10">
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6">
            ...
            >
              <div className="max-w-[1320px] mx-auto px-4 sm:px-6">
                <div ref={headerScrollRef} className="overflow-x-auto no-scrollbar flex w-full">
                  {/* Left Label Spacer */}
                  <div className="w-[140px] sm:w-[200px] min-w-[140px] sm:min-w-[200px] shrink-0 bg-slate-50 border-r border-slate-200/85 p-3 flex items-center font-black text-[10px] text-zinc-400 uppercase tracking-widest">
                    COMPARING
                  </div>
                  ...
                  {displayedItems.map((item, idx) => (
                    <div
                      ...
                      className={cn(
                        "p-3 border-l border-zinc-100 min-w-[240px] sm:min-w-[300px] flex-1 flex items-center justify-between gap-2 bg-white transition-colors duration-150",
                  ...
                  {/* Empty Spacer Column matching table */}
                  {!isMobile && compare.items.length < 4 && (
                    <div className="bg-zinc-50/5 border-l border-zinc-100 min-w-[240px] sm:min-w-[300px] flex-1" />
                  )}
                </div>
              </div>
            </motion.div>
        ...
        <div className="bg-white shadow-xl shadow-slate-100 rounded-3xl border border-slate-100 overflow-hidden">
          <div ref={tableScrollRef} className="overflow-x-auto comparison-scrollbar w-full">
            <table className="w-full border-collapse">
              <tbody>
                {/* Product Header Row */}
                <tr className="border-b border-zinc-100 relative">
                  <td className="p-4 sm:p-6 bg-zinc-50 border-r border-zinc-200/80 w-[140px] sm:w-[200px] min-w-[140px] sm:min-w-[200px] shadow-[4px_0_8px_-3px_rgba(0,0,0,0.06)]">
                    <div className="flex flex-col gap-0.5">
                      <span className="font-black text-[10px] sm:text-[11px] text-zinc-400 uppercase tracking-widest">
                  ...
                  {displayedItems.map((item, idx) => (
                    <td
                      ...
                      onMouseLeave={() => setHoveredColIndex(null)}
                      className={cn(
                        "p-4 text-center border-l border-zinc-100 min-w-[240px] sm:min-w-[300px] bg-white transition-all duration-300 relative py-6 sm:py-8",
                        hoveredColIndex === idx && "bg-indigo-50/5"
                      )}
                    >
                      <div className="flex flex-col items-center gap-3 w-full">
                        {/* Image Block with corner red Close Button */}
                        <div className="relative w-28 h-28 sm:w-36 sm:h-36 bg-zinc-50/50 rounded-2xl p-2 border border-zinc-100/80 flex items-center justify-center">
                          <Image src={item.image} alt={item.name} fill className="object-contain p-2" />
                          <button
                            onClick={() => compare.removeItem(item.id)}
                            className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 border-2 border-white hover:bg-red-600 transition-all z-10 w-6 h-6 flex items-center justify-center cursor-pointer shadow-md"
                        ...
                        <div className="space-y-1 w-full text-center px-1">
                          <Link
                            ...
                            className="text-xs sm:text-sm font-bold text-zinc-900 hover:text-indigo-600 transition-colors line-clamp-2 leading-snug min-h-[36px]"
                          >
                            {item.name}
                          </Link>
                          {/* Price in red */}
                          <div className="text-red-500 font-extrabold text-[14px] sm:text-[16px]">
                            ৳{item.price.toLocaleString()}
                          </div>
                        </div>
                      </div>
                  ...
                  {/* Empty comparison columns placeholder */}
                  {!isMobile && compare.items.length < 4 && (
                    <td className="bg-zinc-50/15 border-l border-zinc-100 p-8 text-center min-w-[240px] sm:min-w-[300px] hover:bg-zinc-50/30 transition-colors">
                      <Link href="/products" className="inline-flex flex-col items-center gap-3 text-zinc-400 hover:text-indigo-600 transition-colors group">
                        <div className="w-12 h-12 rounded-2xl border-2 border-dashed border-zinc-200 flex items-center justify-center group-hover:border-indigo-500 group-hover:bg-indigo-50/50 transition-all">
                          <Plus size={20} className="group-hover:scale-110 transition-transform text-zinc-400 group-hover:text-indigo-600" />
                        </div>
                        <span className="text-[11px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-indigo-600">Add Product</span>
                      </Link>
                    </td>
                  )}
                </tr>
                ...
                {(!hideIdentical || isSpecDifferent("availability")) && (
                  <tr className={cn(
                    ...
                    <td className="sticky left-0 z-20 bg-zinc-50/95 border-r border-zinc-200/80 font-black text-zinc-400 text-[10px] sm:text-[11px] uppercase tracking-widest px-4 py-4 w-[140px] sm:w-[200px] min-w-[140px] sm:min-w-[200px] shadow-[4px_0_8px_-3px_rgba(0,0,0,0.04)]">
                      AVAILABILITY
                    </td>
                    {displayedItems.map((item, idx) => {
                      ...
                      return (
                        <td 
                          ...
                          onMouseLeave={() => setHoveredColIndex(null)}
                          className={cn(
                            "p-4 text-center border-l border-zinc-100 transition-colors duration-150",
                            isDiff ? "bg-[#FFF8E6]" : "bg-white",
                            hoveredColIndex === idx && (isDiff ? "bg-[#FFF3D6]" : "bg-indigo-50/5")
                          )}
                        >
                          <span className="inline-block px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-extrabold rounded-full border border-emerald-100">
                            {item.availability || "In Stock"}
                          </span>
                        </td>
                      );
                    })}
                    {!isMobile && compare.items.length < 4 && <td className="bg-zinc-50/5 border-l border-zinc-100" />}
                  </tr>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      {displayedItems.map((item, idx) => {
                        return (
                          <td 
                            key={`${label.toLowerCase()}-${item.id}`} 
                            onMouseEnter={() => setHoveredColIndex(idx)}
                            onMouseLeave={() => setHoveredColIndex(null)}
                            className={cn(
                              "p-4 text-center border-l border-zinc-100 text-[13px] text-slate-700 font-semibold transition-colors duration-150",
                              isDiff ? "bg-[#FFF8E6]" : "bg-white",
                              hoveredColIndex === idx && (isDiff ? "bg-[#FFF3D6]" : "bg-indigo-50/5")
                            )}
                          >
                ...
                {filteredSpecGroups.map((group) => (
                  <React.Fragment key={group.id}>
                    {/* Group Header Bar */}
                    <tr>
                      <td colSpan={displayedItems.length + 1} className="bg-gradient-to-r from-indigo-50/80 to-white p-0 border-b border-zinc-200/80 relative">
                        <button
                          onClick={() => setCollapsedGroups(prev => ({ ...prev, [group.id]: !prev[group.id] }))}
                          className="sticky left-0 flex items-center justify-between w-full px-4 py-3.5 group/btn text-left"
                        >
                          <div className="flex items-center gap-3">
                            <div className="p-1.5 rounded-lg bg-indigo-100/50 text-indigo-600">
                              <group.icon size={14} strokeWidth={2.5} />
                            </div>
                            <span className="text-[12px] font-black text-indigo-950 uppercase tracking-widest">
                              {group.name}
                            </span>
                            <span className="text-[10px] text-zinc-400 font-medium normal-case">
                      ...
                      {!isMobile && compare.items.length < 4 && <td className="bg-gradient-to-r from-indigo-50/80 to-white border-l border-zinc-100/50 border-b border-zinc-200/80" />}
                    </tr>
                            ...
                            return (
                              ...
                              >
                                ...
                                {displayedItems.map((item, idx) => {
                                  return (
                                    <td 
                                      key={`${group.id}-${spec.key}-${item.id}`} 
                                      onMouseEnter={() => setHoveredColIndex(idx)}
                                      onMouseLeave={() => setHoveredColIndex(null)}
                                      className={cn(
                                        "p-4 text-center border-l border-zinc-100 text-[13px] text-slate-700 font-semibold leading-relaxed transition-colors duration-150",
                                        isDiff ? "bg-[#FFF8E6]" : "bg-white",
                                        hoveredColIndex === idx && (isDiff ? "bg-[#FFF3D6]" : "bg-indigo-50/5")
                                      )}
                                    >
